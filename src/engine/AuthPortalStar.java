...
public class AuthPortalStar {
	...
	private CharSequence LOGIN_REQUEST_SIGNATURE="在星巴克享受免费无线上网";
	...
	public int login(String user, String password,Context context) {
		...
		try {
			...
			else if (output.contains(cursor.getString(1))) {
				Logger.getInstance().writeLog("HTTP Response:\n" + output);
				Matcher hostMatcher = hostPattern.matcher(aimUrl);
				hostMatcher.find();
				String host = hostMatcher.group(0);
				HttpPost httpPost = new HttpPost(host + SUBMIT_ACTION);
