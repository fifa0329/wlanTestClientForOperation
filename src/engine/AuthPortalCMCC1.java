...
public class AuthPortalCMCC1 {
	...
	private static AuthPortalCMCC1 instance = null;
	...
	public static AuthPortalCMCC1 getInstance() {
		if (instance == null) {
			instance = new AuthPortalCMCC1();
		}
		Log.v("++++++++", "getinstance启动！！！");
		return instance;
	}
	...
	public int login(String user, String password, Context context) {
		...
		try {
			...
			Log.v("========================================",
					getAimUrl(LOGIN_TEST_URL));
			// Log.v("========================================",
			// getCurUrl("http://172.13.0.1:80/auth_index.php?client_mac=%20%200%2038187%2030%2081151&gate_ip=172.13.0.1&pre_url=http%3A%2F%2Fwww.baidu.com%2F"));
			...
			MyClientConnManager connectionManager = new MyClientConnManager(
			...
			/*
			 * if (bssid != null) { if (bssid.equals(((WifiManager) context
			 * .getSystemService(Context.WIFI_SERVICE))
			 * .getConnectionInfo().getBSSID())) { nextAction =
			 * prefs.getString("lastAction", null); response =
			 * client.execute(new HttpPost(nextAction)); output =
			 ...
			 * Matcher codeMatcher = loginCodePattern.matcher(output); if
			 * (codeMatcher.find()) { int code =
			 * parseCode(codeMatcher.group(1)); if (code == 0) { nextAction =
			 * parseAuthenPage(output); // prepare // action // parameters //
			 * for // logout Logger.getInstance()
			 * .writeLog("Fast login success!"); return code; }
			...
			Cursor cursor = mydatabase.getInstance().query(
					"CMCC",
					new String[] { "State", "State_Signature", "Next_Action",
							"Next_URL", "Next_Form_Parameters",
							"EXTRA_Parameters", "Next_State" }, "State=?",
					new String[] { "CMCC_LOGIN_STATE" }, null,
					null, null);
			cursor.moveToFirst();
			if (!output.contains(cursor.getString(1))) {
			...
			if (output.contains(cursor.getString(1))) {
				...
				if (codeMatcher.find()) {
					...
					if (code == 0) {
						nextAction = parseAuthenPage(output); // prepare action
																// parameters
																// for logout
	...
	public static String getAimUrl(String next) {
		...
		Matcher hostMatcher = hostPattern.matcher(next);
		hostMatcher.find();
		return hostMatcher.group(0);
