...
public class AuthPortalCMCC {
	...
	public String getDescription(int code) {
		...
		case RET_UNKNOWN:
			...
			break;
		case RET_ALREADY:
			ret = "“—æ≠‘⁄œﬂ£¨Œﬁ–Ë÷ÿ∏¥µ«¬º£® «∑Ò…œ¥Œµ«¬ºŒ¥œ¬œﬂ£ø£©";
			break;
		case 0:
			ret = "≤Ÿ◊˜≥…π¶";
			break;
		...
		case 105:
			...
			break;
		case 106:
			ret = "»œ÷§«∞ÃﬂÕ¨“ª”√ªßœ¬œﬂ ß∞‹£¨«Î¡™œµ10086";

			break;
		case 107:
			ret = "ƒ˙µ±«∞‘⁄œﬂ÷’∂À ˝“—æ≠≥¨π˝3∏ˆ";
	...
	public int login(String user, String password, Context context) {
		...
		try {
			...
			if (bssid != null) {
				if (bssid.equals(((WifiManager) context
					...
					if (codeMatcher.find()) {
						...
						if (code == 0) {
							nextAction = parseAuthenPage(output); // prepare
																	// action
																	// parameters
																	// for
																	// logout
							Logger.getInstance()
									.writeLog("Fast login success!");
							return code;
						}
						Logger.getInstance()
								.writeLog("Fast login code=" + code);
					}
				}
			}
